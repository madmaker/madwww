...
class uCat_art_files {
    ...
    private function check_data() {
        //uCat/cat_files/site_id/art_id/file_id/file_name
        //$this->uCore->url_prop[1]
        //1 - site_id  - не используется. Просто в старых адресах было
        //2 - art_id - key
        //2 - file_id - key. В БД искать нужно по where art_id and file_id. Они связаны
        //3 - file_name - не используется. Просто в старых адресах было

        uFunc::journal('request: '.$_SERVER['REQUEST_URI'].' ---- referrer: '.(isset($_SERVER['HTTP_REFERER'])?$_SERVER['HTTP_REFERER']:''),'uCat_old_art_files_requests');
        ...
        $this->art_id=$this->uCore->url_prop[2];
        if(!uString::isDigits($this->art_id)) return $this->return_error('Файл не найден','Такой файл не найден',3);
        $this->art_id=(int)$this->art_id;
