...
class item_avatar {
    ...
    private function checkData() {
        ...
        if(!$this->img_time&&!$this->var_id) {
            try {
                ...
                ");
                $site_id=site_id;
                $stm->bindParam(':item_id', $this->item_id,PDO::PARAM_INT);
            ...
            if($qr=$stm->fetch(PDO::FETCH_OBJ)) {
                $this->img_time = (int)$qr->item_img_time;
            }
        }
        elseif(!$this->img_time&&$this->var_id) {
            try {
                ...
                ");
                $site_id=site_id;
                $stm->bindParam(':var_id', $this->var_id,PDO::PARAM_INT);
                $stm->bindParam(':item_id', $this->item_id,PDO::PARAM_INT);
                $stm->bindParam(':site_id', $site_id,PDO::PARAM_INT);
                $stm->execute();
            }
    ...
    private function update_var_img_time($default_var_id) {
        ...
        try {
            ...
            ");
            $site_id=site_id;
            $stm->bindParam(':img_time', $this->img_time,PDO::PARAM_INT);
            $stm->bindParam(':item_id', $this->item_id,PDO::PARAM_INT);
...
//                $orig_img='images/uCat/item_def_avatar.jpg';
            ...
            if (!file_exists($dir2) && !mkdir($dir2, 0755, true) && !is_dir($dir2)) {
            ...
            if(isset($im)) {
                ...
                if ($this->img_width !== 'orig') {
                    $im->resizeImage($this->img_width, 0, Imagick::FILTER_LAGRANGE, 1);
                }
