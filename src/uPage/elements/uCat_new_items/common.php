...
class uCat_new_items {
    private $uFunc;
    private $uPage;
    private $uCore;

    public function text($str) {
        return $this->uCore->text(array('uPage','uCat_new_items'),$str);
    }
    ...
    private function create_default_el_settings($cols_els_id,$site_id=site_id) {
        ...
        catch(PDOException $e) {$this->uFunc->error('uCat_new_items 10'/*.$e->getMessage()*/);}
    }
    public function get_el_settings($cols_els_id,$site_id=site_id) {
        ...
        catch(PDOException $e) {$this->uFunc->error('uCat_new_items 20'/*.$e->getMessage()*/);}
        return 0;
    }
    ...
    public function copy_el($cols_els_id,$new_col_id,$el,$source_site_id=site_id,$dest_site_id=0) {
        ...
        //attach sects to col
        $this->uPage->create_el($cols_els_id,$new_col_id,'uCat_new_items',$el->el_pos,$el->el_style,$el->el_id,$dest_site_id);

        try {
            /** @noinspection PhpUndefinedMethodInspection */
            $stm=$this->uFunc->pdo("uPage")->prepare("INSERT INTO el_config_uCat_new_items (
            cols_els_id,
            site_id,
            items_number,
            dots_style,
        ...
        catch(PDOException $e) {$this->uFunc->error('uCat_new_items 30'/*.$e->getMessage()*/);}
        return $cols_els_id;
    }
    ...
    public function attach_el2col($col_id,$el_id) {
        ...
        //attach sects to col
        $res=$this->uPage->add_el2db($cols_els_id,$el_pos,'uCat_new_items',$col_id,$el_id);
    ...
    public function load_el_content($cols_els_id,$site_id=site_id) {
        ...
        $cnt=$this->uCat->new_items_widget($cols_els_id);
    ...
    public function save_el_conf($cols_els_id) {
        ...
        catch(PDOException $e) {$this->uFunc->error('uCat_new_items  40'/*.$e->getMessage()*/);}
