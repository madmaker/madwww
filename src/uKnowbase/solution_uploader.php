...
class uKnowbase_solution_uploader {
    public $uFunc;
    public $uSes;
    ...
    private function checkData() {
        ...
        if(!$this->uCore->access(38)) {
            ...
            `site_id`='".site_id."'
            ")) $this->uCore->error(1);
            if(!mysqli_num_rows($query)) $this->finish('error','check','You have no rights to upload files');
        }
    ...
    private function after_uploaded_fs_work() {
        ...
        if(!uFunc::create_empty_index($this->uCore->mod.'/'.$this->folder.'/'.$this->file_id)) $this->uCore->error(6);
...
$newClass=new uKnowbase_solution_uploader ($this);
