...
class get_variants {
    ...
    function __construct (&$uCore) {
        ...
        $this->q_variants=$this->uCat_common->get_item_variants_pdo($this->item_id);
...
$uCat=new get_variants($this);
$inaccurate_price_descr=strip_tags($this->uFunc->getConf('inaccurate_price_descr','uCat'))?>
...
<table class="table" id="uCat_item_get_variants_bg_table">
    <tr>
        ...
        <th>Запрашивать цену <span class="icon-question uTooltip" title="Вместо кнопки купить отображать кнопку Запросить цену"></span> </th>
    ...
    <?
        for(;$var;$var=$uCat->q_variants->fetch(\PDO::FETCH_OBJ)){
            $item_type=(int)$uCat->uCat_common->var_type_id2data($var->var_type_id)->item_type_id;
            $base_type_id=(int)$uCat->uCat_common->item_type_id2data($item_type)->base_type_id;?>
            <tr class="<?=$var->default_var=='1'?'active':''?> uCat_item_get_variants_bg_var" id="uCat_item_get_variants_bg_var_<?=$var->var_id?>">
                ...
                <td>
                    <span  id="uCat_admin_item_get_variants_var_type_id_<?=$var->var_id?>"
                        ...
                        data-save_action="uCat_item_admin.variant_var_type_id_update(value,<?=$var->var_id?>)"
                        data-selectbox_values="<?=rawurlencode($uCat->uCat_common->get_var_types_json_not_added2item($var->var_type_id,$uCat->item_id,1));?>"
                        >
                        <?=\uString::sql2text($uCat->uCat_common->var_type_id2data($var->var_type_id)->var_type_title,1)?> <span class="text-muted">(<?=$uCat->uCat_common->item_type_id2data($item_type)->type_title?>)</span>
                    </span>
                </td>
                <td>
                ...
                <?}?>
                <td><input type="checkbox" id="uCat_price_switcher_inaccurate_price_<?=$var->var_id?>" class="uCat_price_switcher_inaccurate_price_bootstrap-switch" <?=(int)$var->inaccurate_price?'checked':''?>></td>
                <td><input type="checkbox" id="uCat_price_switcher_request_price_<?=$var->var_id?>" class="uCat_price_switcher_request_price_bootstrap-switch" <?=(int)$var->request_price?'checked':''?>></td>
                <?if($base_type_id==0) {?>
                    <td id="uCat_admin_item_get_variants_avail_id_file_id_<?=$var->var_id?>" class="<?=$uCat->uCat_common->avail_type_id2class($uCat->uCat_common->avail_id2avail_data($var->avail_id)->avail_type_id)?>">
                        <?
                        $q_avails=$uCat->uCat_common->get_avails();
                        $avail_str='{';
                        ...
                        <span id="uCat_admin_item_get_variants_avail_id_<?=$var->var_id?>"
                            ...
                            >
                            <?=$uCat->uCat_common->avail_id2avail_data($var->avail_id)->avail_label?>
                        </span>
                    </td>
                <?}
                else {?>
                    <td id="uCat_admin_item_get_variants_avail_id_file_id_<?=$var->var_id?>">
                        <div id="uCat_admin_item_get_variants_file_id_<?=$var->var_id?>">
                            <?=$var->file_id=='0'?'<div class="well-sm bg-danger uTooltip" title="Такой вариант товара невозможно купить"><span class="icon-attention"></span> Файл не указан</div>':
                                ('<a target="_blank" href="'.u_sroot.'uDrive/file/'.$var->file_id.'/'.$uCat->uDrive_common->file_id2data($var->file_id)->file_hashname.'">'.$uCat->uDrive_common->file_id2data($var->file_id)->file_name.'</a>')?>
                            <div class="clearfix" style="margin-top: 2px">
                                <button class="btn btn-sm btn-primary" onclick="uCat_item_admin.set_file_init(<?=$var->var_id?>,<?=$var->file_id?><?=(int)$var->file_id?(',\''.rawurlencode($uCat->uDrive_common->file_id2data($var->file_id)->file_hashname).'\',\''.rawurlencode($uCat->uDrive_common->file_id2data($var->file_id)->file_name).'\''):''?>)">Указать файл</button>
                            </div>
                        </div>
                    </td>
                <?}?>
...
<?}
    ...
    <div class="jumbotron">
        <p>У этого товара пока только один вариант. Вы можете <a href="javascript:void(0)" class="btn-link" onclick="uCat_item_admin.new_variant_dg()">добавить еще</a></p>
    </div>
    <div class="bs-callout bs-callout-default">
        <p>Можно создавать несколько вариантов одного и того же товара.<br>
            Варианты могут отличаться по цене, типу товара (ссылка для скачивания для электронных книг, обычный товар и т.п.).</p>
