<?php
class uEvents_event_file_uploader {
    private $uCore,$event_id,$folder,$targetDir,$orig_file_name,$filename,$save_file_name,$file_ext,$file_id,$source_filename,$mime_type,$file_size;
    ...
    private function checkData() {
        ...
        if(!isset($_REQUEST['hashId'],$_REQUEST['hash'],$_REQUEST['event_id'])) $this->finish("error", "check", "Haven't got all data required");

        $this->event_id=$_REQUEST['event_id'];
        if(!uString::isDigits($this->event_id)) $this->finish("error", "check","Handled id error");
        ...
        $this->folder='events_files/'.site_id.'/'.$this->event_id;
    ...
    private function uploader() {
        ...
        else $this->finish('continue', '',$this->text('Part'/*Часть*/).' '.++$chunk.' '.$this->text('of'/*из*/).' '.$chunks);
    }
    ...
    private function text($str) {
        return $this->uCore->text(array('uEvents','admin_event_file_uploader_bg'),$str);
    }
...
$uEvents=new uEvents_event_file_uploader ($this);
