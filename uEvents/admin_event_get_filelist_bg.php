<?php
class uEvent_event_filelist {
    private $uCore;
    public $q_files,$event_id;
    public function text($str) {
        return $this->uCore->text(array('uEvents','admin_event_get_filelist_bg'),$str);
    }
...
$uEvent=new uEvent_event_filelist($this);
...
if(mysqli_num_rows($uEvent->q_files)) {
    ...
    <?while($data=$uEvent->q_files->fetch_assoc()) {
        ...
        ?>
        <div class="col-sm-3 col-md-2" id="uEvents_event_fManager_file_<?=$data['file_id']?>" onclick="
            uEvents_event_admin.insertTinyMCEUrl('<?='uEvents/events_file/'.$data['file_id']?>',<?=$data['file_id'];?>,'<?=$fileExt?>')
            ">
            <div class="thumbnail fancybox uTooltip" title="<?=uString::sql2text($data['file_name'],1)?>" href="<?='uEvents/events_file/'.$data['file_id']?>" style="overflow:hidden; height:<?=$height?>px;"><img src="<?=u_sroot?><?
                ...
                if(in_array($fileExt,$image_f_types)) echo 'uEvents/events_file/'.$data['file_id'].'/sm';
...
<?}
    ...
    <p><?=$uEvent->text("No files found"/*Пока файлов нет*/)?></p>
<?}
